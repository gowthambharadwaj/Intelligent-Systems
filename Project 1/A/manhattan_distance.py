...
def calculate(board, goal):
    ...
    g_n     = 0
    h_n     = manhDist(coordinate(board), goalcord) #Calculate Manhattan distance between the initial state and the goal state
        ...
        for m in moves:
            if loc not in m['pos']:
                ...
                if ~(np.all(list(state['board']) == succ, 1)).any():    #Check if the new configuration is not generated before
                    ...
                    h_n = manhDist(coordinate(succ), goalcord)      #Calculate the Manhattan distance
...
def body():
    print()
    print ("Solution to 8-Puzzle problem using Manhattan distance approach\n")
    ...
    goal = [int(x) for x in input().split()]        # Enter the desired input state. Leave a blank space in between the input characters. Add '0' for the blank space
